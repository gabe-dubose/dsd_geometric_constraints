...
paramaters = ['a', 'b', 'c', 'd', 'Du', 'Dv']
i, j = 0, 1 # --- can change to vary which parameters are used
paramater_i_range = np.linspace(0.9*theta[i], 1.2*theta[i], 30)
parameter_j_range = np.linspace(0.9*theta[j], 1.2*theta[j], 30)
X, Y = np.meshgrid(parameter_j_range, paramater_i_range)
Z = np.zeros_like(X)

# flatten grid for parallel runs
points = [(value_i, value_j) for value_i in paramater_i_range for value_j in parameter_j_range]
...
ax1.set_xlabel(rf'${paramaters[j]}$')
ax1.set_ylabel(rf'${paramaters[i]}$')
ax1.set_zlabel(r'Fitness ($W$)')
ax1.view_init(elev=25, azim=105)
plt.tight_layout()
plt.savefig('../figures/fitness_ridge.png', dpi=600)
